    ...
    <section id="why-us" className="py-20 md:py-28 bg-[#07132F] text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="max-w-2xl mx-auto text-center mb-16">
          ...
          <p className="text-sm sm:text-base text-[#8DBCE2] font-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          {/* Right: 4 Key Strengths Cards */}
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {SITE_CONFIG.strengths.map((item) => (
              <div
                key={item.number}
                className="group p-6 rounded-2xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-[#5BB8F5]/40 backdrop-blur-md transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <ShieldCheck className="w-4 h-4 text-[#5BB8F5]/60 group-hover:text-[#5BB8F5] transition-colors" />
                  </div>

                  <h3 className="font-serif text-xl font-medium text-white mb-2 leading-snug group-hover:text-[#7CD0FF] transition-colors">
                    {item.title}
                  </h3>

                  <p className="text-xs text-[#8DBCE2] font-light leading-relaxed mb-4">
                    {item.desc}
                  </p>
                </div>
                ...
                <div className="pt-3 border-t border-white/10 space-y-1.5">
                  {item.points.map((pt, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-[11px] text-slate-300">
                      <span className="w-1 h-1 rounded-full bg-[#5BB8F5]" />
                      <span>{pt}</span>
                    </div>
                  ))}
                </div>
